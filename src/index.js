...
async function displayVideos(videos){
  ...
  for(let i = 0; i < Categories.length; i++){
    Headers[i] = element('div', {'class' : 'row'}, {}, Categories[i].title);
    ...
    for(let j = 0; j < Videos[i].length; j++){
      ...
      Thumbnails[i][j] = element('img', {'src' : videos.videos[videoId].poster}, {}, '');
      Videos[i][j] = element('div', {'class' : 'col col-4 col-sm-10'}, {}, Thumbnails[i][j]);
