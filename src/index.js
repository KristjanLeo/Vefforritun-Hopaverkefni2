...
async function displayVideos(videos) {
  ...
  for (let i = 0; i < videos.categories.length; i += 1) {
    Categories[i] = videos.categories[i];
  }

  // Búum núna til grid (div) fyrirsérhvern flokk, header row og 1 eða 2
  // aukalega til þess að halda utan um videoin
  ...
  for (let i = 0; i < Categories.length; i += 1) {
    ...
    for (let j = 0; j < Videos[i].length; j += 1) {
